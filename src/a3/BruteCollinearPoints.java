...
public class BruteCollinearPoints {
	...
	public BruteCollinearPoints(Point[] points) {
		...
		this.lines = new LineSegment[(int) Math.pow(points.length, 4)];
	...
	protected Point smallestPoint(Point p1, Point p2, Point p3, Point p4) {
		Point smallest;
	...
	protected Point largestPoint(Point p1, Point p2, Point p3, Point p4) {
		Point largest;
		if(p1.compareTo(p2) <= 0 && p1.compareTo(p3) <= 0 && p1.compareTo(p4) <= 0) {
			largest = p1;
		} else if (p2.compareTo(p3) <= 0 && p2.compareTo(p4) <= 0) {
